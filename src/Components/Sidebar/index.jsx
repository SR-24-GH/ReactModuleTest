  ...
  return (
    <div className='side-container'>
      ...
      <div className='create-btn'>
        <button className='create-head' onClick={() => setCreate(!create)}><span className='plus'>+</span></button>
        {sideHead.map((each, index) => {
          ...
          return (
            <ul className='side-list' style={{backgroundColor: `${activate && each.title === activate ? 'rgba(47, 47, 47, 0.17)' : 'white'}`, opacity: `${create ? '5%' : '100%'}`}} key={index}>
              <div className='list-container' onClick={() => setActivate(each.title)}>
                <div style={{backgroundColor: each.color, borderRadius: '100%'}} className='list-icon-div'><span className='list-icon'>{each.title.split(" ").length>1 ? each.title.split(" ")[0][0].toUpperCase() + each.title.split(" ")[1][0].toUpperCase() : each.title.split(" ")[0][0].toUpperCase() + 'G'}<p className='list-title'>{each.title}</p></span></div>
              </div>
            </ul>
        ...
        {create && <div className='newcreate-container'>
            <p className='newcreate-head'>Create New group</p>
            <label className='input-label' htmlFor='create'>Group Name</label>
            <input type='text' id='create' name='create' onInput={(e) => setTitle(e.target.value)} placeholder='Enter group name'></input>
            <div className='color-div' style={{display: 'flex', flexDirection: 'row', gap: '1vw'}}>
              <p className='color-para'>Choose colour</p>
              {colors.map((eachColor, index) => <div className='color-each' key={index} onClick={() => handleColor(eachColor)} style={{backgroundColor: eachColor, width: '40px' , height: '40px', borderRadius: '50%', cursor: 'pointer'}}></div>)}
            </div>
